...
let
  ...
  cfg = if exists then config.programs.${program} or { } else { };
  ...
  paths =
    ...
    in
    {
      ...
      downloads = if isNotEmpty user.paths.downloads then "${homeDir}/${user.paths.downloads}" else "${homeDir}/Downloads";
    };
