...
class WeatherCache:
    ...
    def get(self, location, logger, ignore_cache=False):
        ...
        # Check memory cache
        if (
            cache_key in self.memory_cache
            and datetime.now().timestamp() - self.memory_cache.get(time_key, 0)
            ...
            if os.path.exists(self.cache_file):
                with open(self.cache_file, "r") as f:
                    ...
                    if (
                        ...
                        and datetime.now().timestamp() - file_cache.get(time_key, 0)
                        < self.cache_duration * 60
                    ):
    ...
    def save(self, temperature, location, logger):
        ...
        # Update memory cache
        self.memory_cache.update({cache_key: temperature, time_key: current_time})
...
def main():
    ...
    parser.add_argument(
        "location",
        nargs="?",
        default="Mandeville,Jamaica",
        help="Location to fetch weather for",
    )
    parser.add_argument(
        "--format",
        choices=["1", "2", "3", "4"],
        default="4",
        help="Weather format (1-4)",
    )
