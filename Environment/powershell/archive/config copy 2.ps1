...
function Get-LoadOrder {
  ...
  # Get order files from config, or use defaults
  $orderFiles = $Config.OrderFiles ?? @('.dotsrc', '.dots.toml', '.dots.json', 'dots.json', '.dots.conf', '.dots.txt', 'config.json', 'import.txt', 'load-order.txt')
  ...
  foreach ($orderFile in $orderFiles) {
    ...
    if (Test-Path $orderPath) {
      ...
      try {
        ...
        # Auto-detect format by content and extension
        if (($content.Trim().StartsWith('{') -and $content.Trim().EndsWith('}')) -or $orderFile -like '*.json') {
          # JSON format
          $configContent = $content | ConvertFrom-Json
          return $configContent.loadOrder ?? $configContent.modules ?? $configContent.order
        }
        elseif ($orderFile -like '*.toml') {
          # TOML format - placeholder for future implementation
          Write-Warning "$ctx_tag TOML parsing not yet implemented for $orderFile"
          continue
        }
        else {
          # Plain text format
    ...
    if (Test-Path $configPath) {
      try {
        ...
        if ([string]::IsNullOrWhiteSpace($content)) {
          Write-Debug "$ctx_tag   Local config requests skip (empty file): $configFile"
          return $true
        }
        ...
        if (($content.StartsWith('{') -and $content.EndsWith('}')) -or $configFile -like '*.json') {
          try {
            ...
            if ($localConfig.skip -eq $true) {
              Write-Debug "$ctx_tag   Local config requests skip (JSON): $configFile"
              return $true
            }
          }
          catch {
        ...
        # Handle as plain text - check for "skip" keyword
        if ($content -eq 'skip') {
          Write-Debug "$ctx_tag   Local config requests skip (text): $configFile"
          return $true
        }

        # TODO: Add TOML parsing when needed
        # For now, if it's not JSON and not "skip", assume it's load order content
...
function Import-DirectoryScripts {
  ...
  if (Test-ShouldSkip -DirectoryPath $DirectoryPath) {
