...
function Get-LoadOrder {
  ...
  if ($localConfig.Includes) {
    Write-Debug "$ctx_tag   Found includes in local config"
    return $localConfig.Includes
  }
...
function Import-Script {
  ...
  try {
    if ($ScriptPath -like '*.psm1') {
      ...
      # Use Global scope to ensure functions are available globally
      . $Global:ExecutionContext.InvokeCommand.InvokeScript($false, (Get-Content $ScriptPath -Raw), $null, $null)
    ...
    if (Test-Path $configPath) {
      try {
        ...
        if (($content.StartsWith('{') -and $content.EndsWith('}')) -or $configFile -like '*.json') {
          try {
            ...
            return @{
              ...
              Excludes = $localConfig.Excludes ?? $localConfig.excludes ?? $localConfig.exclude ?? @()
              Includes = $localConfig.Includes ?? $localConfig.includes ?? $localConfig.loadOrder ?? $localConfig.modules ?? $localConfig.order
            }
          }
          catch {
        ...
        elseif ($configFile -like '*.toml') {
          ...
          Write-Warning "$ctx_tag TOML parsing not yet implemented for $configFile"
          continue
        }
...
function Import-DirectoryScripts {
  ...
  if ($loadOrder) {
    Write-Debug "$ctx_tag$indent Using custom includes ($($loadOrder.Count) items)"
