...
test_home=$(mktemp -d)
cleanup() {
  HOME="$test_home" nix develop .#Victus-ai-nine-router --command nine-router-stop >/dev/null 2>&1 || true
  HOME="$test_home" nix develop .#Victus-ai-headroom --command headroom-stop >/dev/null 2>&1 || true
  rm -rf "$test_home"
}
trap cleanup EXIT HUP INT TERM

HOME="$test_home" nix develop .#Victus-ai-nine-router --command sh -lc '
  set -eu
  test "$NINE_ROUTER_PORT" = 20129
  test "$NINE_ROUTER_BIND_ADDRESS" = 127.0.0.1
  test "$NINE_ROUTER_BASE_URL" = http://127.0.0.1:20129/v1
  test "$NINE_ROUTER_DATA_DIR" = "$HOME/.local/share/ai/nine-router"
  command -v nine-router >/dev/null
  command -v nine-router-daemon >/dev/null
  command -v nine-router-status >/dev/null
  nine-router --help >/dev/null
'
...
HOME="$test_home" nix develop .#Victus-ai-hermes-hindsight-nine-router --command sh -lc '
  set -eu
  test "$AI_PRESET" = hermes-hindsight-nine-router
  test "$NINE_ROUTER_DATA_DIR" = "$AI_HOME/nine-router"
  test "$NINE_ROUTER_SESSION" = hermes-hindsight-nine-router-default-nine-router
...
HOME="$test_home" nix develop .#Victus-ai-hermes-hindsight-headroom-nine-router --command sh -lc '
  set -eu
