...
execute() {
  ...
  execute__usage() {
    ...
    *) printf "ERROR: Invalid option: '%s'.\n" "$1" ;;
    ...
    printf "  LIST                List containing items, separated by space or newline\n"
  ...
  execute__main() {
    ...
    case $(separate_by_space "$execute__list") in
    *" $execute__item "*)
      ...
      execute__exec="$(
        separate_by_space "$(
          printf "%s %s %s %s\n" \
            "$(separate_by_space "$execute__base")" \
            "$(separate_by_space "$execute__next")" \
            "$(separate_by_space "$execute__item")" \
            "$(separate_by_space "$execute__last")"
        )"
      )"
...
convert_to_seconds() {
  ...
  case "$unit" in
  h | hour | hours) printf "%s" "$((value * 3600))" ;;
  d | day | days) printf "%s" "$((value * 86400))" ;;
  w | week | weeks) printf "%s" "$((value * 604800))" ;;
  m | month | months) printf "%s" "$((value * 2592000))" ;; # 30 days
  y | year | years) printf "%s" "$((value * 31536000))" ;;
  *) printf "%s" "$value" ;;
  esac
}
...
get_timestamp() {
  _value="$1"
  _unit="$2"
  _secs="$(convert_to_seconds "$_value" "$_unit")"
  _date="$(date +%s)"

  date -d "@$(("${_date}" - "${_secs}"))" +%Y-%m-%d
}
...
set_expiry_date() {
  ...
  nix)
    ...
    *[hdwmy])
      ...
      cutoff_date="$(get_timestamp "${value}" "${unit}")"
  ...
  hm)
    ...
    *[!0-9]*)
      if printf "%s" "$generations" | grep -q "[hdwmy]$"; then
        value="$(printf "%s" "$generations" | sed 's/[hdwmy]$//')"
        unit=$(printf "%s" "$generations" | sed 's/.*\(.\)$/\1/')
        target_timestamp="$(get_timestamp "${value}" "${unit}")"
