...
let
  ...
  collectPackages =
    ...
    let
      normalizePackageName =
        package: package.pname or (package.name or (throw "Expected derivation-like value with pname or name"));

      flattenToList = value: if isAttrs value then attrValues value else toList value;
    in
    listToAttrs (
      map (package: {
  ...
  collectAttrs = { selector, modules }: foldl' (merged: module: merged // (selector module)) { } modules;

  collectMessages =
    { selector, modules }: concatMap (module: optionals ((selector module) != null) (toList (selector module))) modules;
  ...
  collectModules =
    ...
    let
      prioritized = map (module: modules.${module} or (throw "collectModules: unknown module '${module}'")) priority;
  ...
  mkModules =
    ...
    in
    {
      ...
      packages = collectPackages {
        selector = module: module.packages.all or (module.pkgs or (module.packages or [ ]));
        modules = collected.prioritized;
      };

      binaries = collectAttrs {
        selector = module: module.binaries.all or (module.bin or (module.commands or (module.binaries or { })));
      ...
      // collectAttrs {
        selector = module: module.variables or (module.env or (module.environment or { }));
        modules = collected.ordered;
      }
      // foldl' (merged: module: merged // (toVariantJSON (module.cfg or (module.variant or { })))) { } collected.ordered;

      messages = collectMessages {
        selector = module: module.messages or (module.shellHook or (module.shellHookParts or [ ]));
        modules = collected.prioritized;
      };
