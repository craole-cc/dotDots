      ...
      args = {
        config =
          if config != null then
            ...
            {
              allowUnfree = true;
              permittedInsecurePackages = [ "openclaw-2026.3.12" ];
            };
        system = if isNotEmpty system then system else getSystemOrDefault { };
      }
      // optionalAttrs (isNotEmpty inputs) { overlays = mkOverlays { inherit inputs extraOverlays; }; };
    in
    if isEmpty inputs then import <nixpkgs> args else import packages.nix args;

  mkPkgsPerSystem = { inputs, ... }: (genAttrs (defineSystems { })) (system: mkPkgs { inherit inputs system; });
    ...
    let
      errorMsg =
        if isString error then
          "mkPkgs: Critical dependency '${error}' not found in inputs."
        else
          "parseInput: could not resolve one of ${toString names} from inputs.";

      inputs' = optionalAttrs (inputs != null) inputs;
      foundName = findFirst (name: inputs' ? ${name}) null names;
      result = if foundName != null then inputs'.${foundName} else null;
    in
    if foundName == null && isNotEmpty error then throw errorMsg else result;
  ...
  */
    ...
    let
      ...
      fromPath =
        ...
        let
          src = import path;
        in
        if isFunction src then
          src
        else if isAttrs src then
          src.overlays.default or src.overlay or noop
        else
          noop;
    in
    if input == null then
      noop
    else if isFunction input then
      input
    else if isAttrs input && input ? overlays then
      input.overlays.default or noop
    else if isAttrs input && input ? overlay then
      input.overlay
    else if isAttrs input && input ? outPath then
      fromPath input.outPath
    else if isPath input || isString input then
      fromPath input
    else
      noop;
  ...
  perSystem =
    ...
    let
      ...
      packages = if pkgs != null then pkgs else mkPkgsPerSystem { inherit inputs; };
    in
    genAttrs systems (system: fn packages.${system});
in
{
  inherit
    mkOverlays
