      ...
      let
        packages =
          ...
          let
            ...
            custom =
              (optionalAttrs cfg.includePnpm (mkBin {
                inherit pkgs;
                prefix = "pnpm-";
                set = {
                  i = {
                    command = "${pkgs.pnpm}/bin/pnpm install";
                  };
                  a = {
                    script = ''${pkgs.pnpm}/bin/pnpm add "$@"'';
                  };
                  ad = {
                    script = ''${pkgs.pnpm}/bin/pnpm add --save-dev "$@"'';
                  };
                };
              }))
              // (optionalAttrs cfg.includeDeno (mkBin {
                inherit pkgs;
                prefix = "deno-";
                set = {
                  dev = {
                    script = ''${pkgs.deno}/bin/deno task dev "$@"'';
                  };
                  run = {
                    script = ''${pkgs.deno}/bin/deno run "$@"'';
                  };
                  lint = {
                    command = "${pkgs.deno}/bin/deno lint";
                  };
                  fmt = {
                    command = "${pkgs.deno}/bin/deno fmt";
                  };
                  test = {
                    script = ''${pkgs.deno}/bin/deno test "$@"'';
                  };
                  check = {
                    script = ''${pkgs.deno}/bin/deno check "$@"'';
                  };
                };
              }));
            all = common // custom;
