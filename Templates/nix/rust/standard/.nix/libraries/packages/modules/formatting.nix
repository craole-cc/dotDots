...
let
  ...
  mkTreefmt =
    ...
    let
      ...
      sqlFormatterSettings = mapAttrs (_: sqlCfg: {
      ...
      }) (filterAttrs (_: sqlCfg: sqlCfg.enable or false) (cfg.sqlFormatters or { }));
      ...
      // (optionalAttrs cfg.includeRustfmt { rustfmt.enable = true; })
      // (optionalAttrs cfg.includeLeptosfmt { leptosfmt.enable = true; })
      // (optionalAttrs (cfg.includeDeno && for.allBut "riscv64-linux") { deno.enable = true; })
      ...
      config = recursiveUpdate { inherit projectRootFile programs settings; } extraConfig;
