      ...
      let
        ...
        packages =
          let
            common =
              { }
              // (optionalAttrs cfg.includeCodex { codex = llm.codex or pkgs.codex or null; })
              // (optionalAttrs cfg.includeClaude { claude = llm.claude-code or pkgs.claude-code-bin or pkgs.claude-code or null; })
              // (optionalAttrs cfg.includeGemini { gemini = llm.gemini-cli or pkgs.gemini-cli-bin or pkgs.gemini-cli or null; })
              // (optionalAttrs cfg.includeOpenClaw { openclaw = llm.openclaw or pkgs.openclaw or null; })
              // (optionalAttrs cfg.includeHermes { hermes = llm.hermes-agent or pkgs.hermes or null; })
              // { };
        ...
        variables =
          { }
          // optionalAttrs cfg.includeClaude { ANTHROPIC_API_KEY = "$ANTHROPIC_API_KEY"; }
          // optionalAttrs cfg.includeCodex {
            OPENAI_API_KEY = "$OPENAI_API_KEY";
            CODEX_UNSAFE_ALLOW_NO_SANDBOX = "0";
          }
          // optionalAttrs cfg.includeGemini { GEMINI_API_KEY = "$GEMINI_API_KEY"; }
          // optionalAttrs cfg.includeOpenClaw { }
          // optionalAttrs cfg.includeHermes { };
      in
      {
        inherit variables packages binaries;
      }
    );
}
