...
let
  ...
  */
  ensurePrefix = prefix: value: if hasPrefix prefix value then value else prefix + value;
  ...
  */
  ensureSuffix = suffix: value: if hasSuffix suffix value then value else value + suffix;
  ...
  */
  nonEmptyOrNull =
    ...
    if value == null then
      ...
      in
      if stringValue == "" then null else stringValue;
  ...
  */
  concatNonEmpty =
    delim: input:
    if isAttrs delim then
      concatStringsSep (delim.separator or delim.sep or delim.delimiter or "") (
        filter (part: part != null) (map nonEmptyOrNull (delim.parts or delim.list or delim.strings or [ ]))
      )
    else
      concatStringsSep delim (filter (part: part != null) (map nonEmptyOrNull input));
  ...
  toPathString =
    ...
    let
      ...
      mkParts =
        value:
        flatten (
          map (part: if isStringLike part && hasInfix separator part then splitString separator part else [ part ]) (toList value)
        );
