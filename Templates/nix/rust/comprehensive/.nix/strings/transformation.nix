...
let
  ...
  splitWords =
    s: splitString "-" (replaceStrings [ " " "_" ] [ "-" "-" ] (normalizeSymbols (toLower s)));
  ...
  */
  toLowerCase =
    input: if isList input && any isList input then throw msgs.nested else _applyStr toLower input;
  ...
  */
  toUpperCase =
    input: if isList input && any isList input then throw msgs.nested else _applyStr toUpper input;
