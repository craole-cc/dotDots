...
let
          ...
          mainProgram =
            if program != null then
              ...
              drv.meta.mainProgram
                or (drv.NIX_MAIN_PROGRAM or (drv.pname or (if name != null then name else parsed.name)));
        in
  ...
  */
  mkBins =
    ...
    mapAttrs (_: resolveBin) (
      removeAttrs packages (attrNames (filterAttrs (_: v: v == null) packages))
    );
  ...
  */
  ...
  mkCmds =
    bins: f:
    filterAttrs (_: isNotEmpty) (mapAttrs (_: bin: if isNotEmpty bin then f bin else null) bins);
  ...
  mkVr3n =
    ...
    let
      ...
      awk =
        if strip then
          "awk '{print substr(${dollar}${toString field},2)}'"
        else
          "awk '{print ${dollar}${toString field}}'";
    in
    ...
    let
      ...
      mkDiscoveredScript =
        ...
        let
          ...
          runner =
            if ext == "rs" then
              "${pkgs.rust-script}/bin/rust-script"
            else
              source + "/share/${base}/${chosen.name}";
        in
        nameValuePair base (
          pkgs.writeShellScriptBin base ''
            ${scriptEnv ext}
