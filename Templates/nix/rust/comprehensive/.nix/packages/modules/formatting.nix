    ...
    let
      ...
      programs =
        ...
        // optionalAttrs includeSqlfmt {
          sql-formatter = {
            enable = true;
            dialect =
              if configuration.includeSqlite then
                "sqlite"
              else if configuration.includePostgres then
                "postgresql"
              else if configuration.includeMysql then
                "mysql"
              else if configuration.includeMariaDB then
                "mariadb"
              else
                null;
          };
      ...
      settings = {
        ...
        formatter =
          ...
          //
            optionalAttrs
              ((programs ? deno) && programs.deno.enable && (programs ? yamlfmt) && programs.yamlfmt.enable)
              {
                deno.excludes = [
                  "*.yaml"
                  "*.yml"
