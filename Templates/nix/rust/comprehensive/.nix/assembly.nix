  ...
  assemble =
    ...
    foldl' (
      ...
      let
        baseLib = scope acc;
        libForEntry = foldl' (
          depLib: depPath: depLib // (import depPath { lib = depLib; })
        ) baseLib dependencies;
      in
      acc // (importWithFilteredArgs entry ({ lib = libForEntry; } // extraArgs))
    ) start orderedEntries;
    ...
    flatten (
      map (
        ...
        if pathType p == "directory" then
          ...
          optionals (
            pathIsRegularFile p && !(elem (baseNameOf p) ignore) && hasSuffix ".nix" (baseNameOf p)
          ) [ p ]
      ) (toList path)
    );
  ...
  */
  normalizeInput =
    ...
    let
      ...
      // defaults;
    in
    if isPath input || isList input then base // { path = toList input; } else base // input;
  ...
  */
  importAttrs =
    ...
    let
      ...
      libToUse = if n.lib != null then n.lib else lib;
  ...
  */
  importLibs =
    ...
    let
      ...
      libToUse = if n.lib != null then n.lib else lib;
      namespace = if n.namespace != null then n.namespace else inferNamespace n.path;

      #> Default start to the nixpkgs base for this namespace, if it exists

      all = assemble {
        start = if n.start != null then n.start else libToUse.${namespace} or { };
        entries = collectPaths { inherit (n) path recurse ignore; };
        scope = if n.scope != null then n.scope else acc: libToUse // { ${namespace} = acc; };
        priority = n.priority or [ ];
        ignore = n.ignore or [ ];
