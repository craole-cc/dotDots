        ...
        custom = with binaries.common;
          ...
          // optionalAttrs includePostgres (
            mkBin {
              ...
              sep = "-";
              set = {
                connect = {script = ''${postgresql}/bin/psql "$@"'';};
                dump = {script = ''${postgresql}/bin/pg_dump "$@"'';};
                restore = {script = ''${postgresql}/bin/pg_restore "$@"'';};
                start = {script = ''${postgresql}/bin/pg_ctl start "$@"'';};
                stop = {script = ''${postgresql}/bin/pg_ctl stop "$@"'';};
                status = {command = "${postgresql}/bin/pg_ctl status";};
              };
            }
          )
          // optionalAttrs includeMariaDB (
            mkBin {
              ...
              sep = "-";
              set = {
                connect = {script = ''${mysql}/bin/mysql "$@"'';};
                dump = {script = ''${mysqldump}/bin/mysqldump "$@"'';};
                import = {script = ''${mysql}/bin/mysql "$@" < /dev/stdin'';};
                start = {script = ''${mysqld_safe}/bin/mysqld_safe "$@" &'';};
              };
            }
          )
          ...
          // optionalAttrs includeSqlite (
            mkBin {
              ...
              sep = "-";
              set = {
                tables = {script = ''${sqlite} "$1" ".tables"'';};
                schema = {script = ''${sqlite} "$1" ".schema"'';};
              };
            }
          );
