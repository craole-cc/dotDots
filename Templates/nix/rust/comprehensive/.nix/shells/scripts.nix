{
  ...
  mkPackage = {
    ...
    envLines = toLines (mapAttrsToList (name: value: "export ${name}=${escapeShellArg value}") env);
  in
    throwIf (script == null && text == "") "mkPackage: no script found for '${name}'" (
  ...
  mkMissionControl = {
    ...
    commandList = toLines (map (name: "  ${name}  ${commands.${name}.description}") names);
