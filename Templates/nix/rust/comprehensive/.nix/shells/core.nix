  ...
  */
  mkShell =
    ...
    let
      ...
      pkgs' = if pkgs != null then pkgs else mkPkgs { inherit inputs system; };
        ...
        // {
          default =
            if default == null then
              ...
              in
              if found != null then found else throw "mkShells: no shells defined and no default provided."
  ...
  mergeSpecs =
    ...
    let
      meta = {
        ...
        label = {
          left =
            if meta.name.left != null then
              meta.name.left
            else if meta.name.right != null then
              meta.name.right
            else
              null;
          right =
            if meta.kind.left != null then
              meta.kind.left
            else if meta.kind.right != null then
              meta.kind.right
            else
              null;
        };
      ...
      name =
        if isNotEmpty meta.label.left && isNotEmpty meta.label.right then
          concatStringsSep "+" [
            meta.label.left
            meta.label.right
          ]
        else if isNotEmpty meta.label.left then
          meta.label.left
        else if isNotEmpty meta.label.right then
          meta.label.right
        else
          "unnamed";

      kind =
        if isNotEmpty meta.kind.left && isNotEmpty meta.kind.right then
          concatStringsSep "+" [
            meta.kind.left
            meta.kind.right
          ]
        else if isNotEmpty meta.kind.left then
          meta.kind.left
        else if isNotEmpty meta.kind.right then
          meta.kind.right
        else
          "merged";
    in
    {
      ...
      shell = {
        ...
        shellHook = optionalString (isNotEmpty meta.shellHook.left || isNotEmpty meta.shellHook.right) (
          if isNotEmpty meta.shellHook.left && isNotEmpty meta.shellHook.right then
            concatStringsSep "\n" [
              meta.shellHook.left
              meta.shellHook.right
            ]
          else if isNotEmpty meta.shellHook.left then
            meta.shellHook.left
          else
            meta.shellHook.right
        );
    ...
    let
      ...
      mkSpec =
        ...
        // {
          ...
          shell = merged.shell // {
            name = concatStringsSep "+" partNames;
          };
        };
      ...
      mkSuite =
        ...
        let
          subSuites = map (n: optionalAttrs (ns.${n} ? mkSuite) ns.${n}.mkSuite { inherit pkgs; }) names;
        in
