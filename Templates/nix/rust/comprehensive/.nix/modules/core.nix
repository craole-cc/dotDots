...
let
  ...
  collectPackages =
    ...
    let
      normalizeName =
        pkg: pkg.pname or (pkg.name or (throw "collectPackages: derivation missing pname and name"));
    in
    listToAttrs (
      map (pkg: {
  ...
  collectAttrs =
    ...
    foldl' (merged: m: merged // (selector m)) { } modules;
  ...
  collectMessages =
    ...
    concatMap (m: optionals ((selector m) != null) (toList (selector m))) modules;
  ...
  collectModules =
    ...
    let
      ...
      prioritized = map (
        k: normalizeModule (modules.${k} or (throw "collectModules: unknown module '${k}'"))
      ) priority;
