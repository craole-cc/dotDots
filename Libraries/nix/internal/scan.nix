...
let
  ...
  withArticle =
    ...
    if word == "null" then
      ...
      in
      if elem firstLetter vowels then "an ${word}" else "a ${word}";
  ...
  concatNonEmptyStrings =
    ...
    concatStringsSep sep (filter (string: string != "") (toList parts));
  ...
  foldModule =
    ...
    let
      ...
      values = map (key: module.${key} or null) ((toList aliases) ++ [ name ]);
      present = filter (value: value != null) values;
    in
    if canonical == "string" then
      concatNonEmptyStrings {
        sep = "\n";
        parts = map (
          value:
    ...
    else if canonical == "set" then
      foldl' (a: b: a // b) { } present
    else if canonical == "list" then
      foldl' (a: b: a ++ b) [ ] present
    else
      throw "foldModule: unsupported type '${typeAliases.display.${type} or (withArticle type)}'";
  ...
  assertType =
    ...
    let
      ...
      display = {
        actual = typeAliases.display.${result.actual} or (withArticle result.actual);
        expected = typeAliases.display.${result.expected} or (withArticle result.expected);
      };
    in
    if result.actual == result.expected then
      value
    else
      throw "${name}: expected ${display.expected}, got ${display.actual}";
  ...
  isExcludedFile =
    file: elem file exclusions.files || (foldl' (acc: pat: acc || hasSuffix pat file) false exclusions.patterns);
  ...
  findDocs =
    ...
    let
      ...
      found = findFirst (path: path != null && pathExists (toString path)) null candidates;
    in
    if found != null then
      {
        type = "markdown";
          ...
          tests =
            ...
            if available then
              let
                results = foldlAttrs (
                  acc: _: group:
                  acc ++ (optionals (isAttrs group) (map (n: group.${n}) (attrNames group)))
                ) [ ] normalized.__tests;
  ...
  scanDir =
    ...
    let
      processEntry =
        entryName: entryType:
        if entryType == "directory" && isExcludedDir entryName then
          empty
          ...
          in
          {
            modules = if res.modules != { } then { ${entryName} = res.modules; } else { };
            inherit (res) rootAliases;
          }
        else if entryType == "regular" && hasSuffix ".nix" entryName && !isExcludedFile entryName then
          processNixFile dir pathPrefix entryName
        else
          empty;
