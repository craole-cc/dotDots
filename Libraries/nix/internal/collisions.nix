...
let
  ...
  validated =
    if elem collisionStrategy allowed then
      collisionStrategy
    else
      throw ''
...
if !hasCollisions then
  base // overrides
else if validated == "error" then
  throw fullMsg
else if validated == "warn" then
  trace "WARNING: ${fullMsg}" (base // overrides)
else if validated == "prefer-custom" then
  base // overrides
else
  overrides // base
