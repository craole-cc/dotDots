...
let
  ...
  capitalize =
    input:
    optionalString (input != "" && input != null) (
      "" + toUpper (substring 0 1 input) + substring 1 (stringLength input - 1) input
    );

  uncapitalize =
    input:
    optionalString (input != "" && input != null) (
      "" + toLower (substring 0 1 input) + substring 1 (stringLength input - 1) input
    );

  toPascal = input: concatStrings (map capitalize (splitString "-" input));
  ...
  mkModuleExports =
    ...
    let
      ...
      suffix = optionalString (filename != null && filename != "") (toPascal filename);
      ...
      splitFnPrefix =
        ...
        let
          ...
          prefixes' = filter (prefix: hasPrefix prefix stem) (uniqueStrings (prefixes ++ commonPrefixes));
          isPrefixed = prefixes' != [ ];
          prefix = optionalString isPrefixed (head prefixes');
        in
        {
          inherit prefix;
          stem = if isPrefixed then removePrefix prefix stem else stem;
        };

      mkAliases =
        fns:
        optionalAttrs (suffix != "") (
          listToAttrs (
            map (fn: {
              name = fn + suffix;
              value = fns.${fn};
