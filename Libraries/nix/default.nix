  ...
  flake' =
    if flake != null then
      ...
      let
        ...
        inherit (paths') src;
      in
      if pathExists (toString src + "/flake.nix") then getFlake (toString src) else { }
    else
      { };
...
in
import ./internal {
  collisionStrategy = if collisionStrategy != null then collisionStrategy else defaults.collisionStrategy;
