{
  ...
  in {
    fromStable = import (inputs.nixpkgs-stable or inputs.nixpkgs) {inherit system config;};
  };
  ...
  mkHome = {
    ...
    # The application registry resolves Helix from nixpkgs (`inputPath = null`).
    # Keep the flake input available for explicit consumers without allowing its
