...
let
  ...
  */
  getPackages =
    ...
    let
      system' = if system != null then system else currentSystem;

      pkgsBase = if legacyPackages != { } then legacyPackages else flake.legacyPackages or nixpkgs.legacyPackages or { };

      pkgsFor = sys: if sys == null then import <nixpkgs> { } else pkgsBase.${sys} or (import <nixpkgs> { system = sys; });
  ...
  getSystems =
    ...
    let
      ...
      derived =
        if system != null then
          system
        else builtins.currentSystem or (if common != null then
          common
        else
          last default);
