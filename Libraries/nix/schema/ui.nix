...
let
  ...
  inherit (_.options.construction)
    mkOptionEnum
    mkOptionEnums
    mkOption
    mkTrue
  ...
  select =
    ...
    let
      ...
      name = if isAttrs rawVal then rawVal.name or null else rawVal;
    in
    if name != null && hasAttr name set then
      {
        inherit name;
        kind = key;
        config = set.${name};
  ...
  normalize =
    ...
    let
      ...
      environment =
        if windowManager.name != null then
          windowManager
        else if desktopEnvironment.name != null then
          desktopEnvironment
        else
          let
          ...
          optionalAttrs (env != null) (
            if hasAttr env windowManagers then
              {
                name = env;
                kind = "windowManager";
  ...
  requireUI =
    ...
    if ui != null then ui else mkUI { inherit host; };
  ...
  mkDefault =
    ...
    let
      ...
      go =
        opt: val:
        if opt ? type || opt ? description then
          if val != null then opt // { default = val; } else opt
        else
          genAttrs (attrNames opt) (subkey: go opt.${subkey} (if isAttrs val then val.${subkey} or null else null));
    in
