...
let
  ...
  */
  getAllWithDefault =
    ...
    let
      ...
      principal = if all == { } then throw "No users defined" else head (attrNames all);
    in
    if filterAttrs (_: u: u.enable) all == { } then
      all
      // {
  ...
  */
  getPrimary =
    ...
    if autoLogin != { } then
      head (attrValues autoLogin)
    else if elevated != { } then
      head (sort (a: b: a._rank < b._rank) (attrValues elevated))
    else if interactive != { } then
      head (attrValues interactive)
    else
      { };
  ...
  */
  mkHome =
    ...
    let
      ...
      names = {
        all = attrNames all;
        primaryUser = if primary != { } then head (attrNames primary) else null;
