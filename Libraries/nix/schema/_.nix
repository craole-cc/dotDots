  ...
  */
  mkSchema =
    ...
    let
      ...
      users = if paths.users != null then importAttrs paths.users else { };
      hosts =
        if paths.hosts != null then
          mapAttrs (name: host: mkCore { inherit name host users; }) (importAttrs paths.hosts)
        else
          { };
    in
    {
