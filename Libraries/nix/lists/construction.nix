...
let
  ...
  */
  mkCheckList =
    ...
    let
      ...
      normalizedList = if exact then checkList else map toLower checkList;
    in
    if !isList checkList then
      throw (
        _debug.withDoc {
      ...
      (e: elem e normalizedList)
    else
      (e: if e == null then elem null normalizedList else elem (toLower e) normalizedList);
  ...
  */
  mkDenyValidator =
    ...
    if !isList list then
      ...
      {
        check = name: if exact then !(isInExact name list) else !(isIn name list);
        inherit list;
      };
  ...
  */
  mkEnum =
    input:
    if !(isList input || isAttrs input) then
      throw (
        _debug.withDoc {
          function = "mkEnum";
      ...
      let
        normalized =
          if isList input then
            { values = input; }
          else if input ? values then
            input // { values = if isList input.values then input.values else attrNames input.values; }
          else
      ...
      in
      {
        inherit
          ...
          ;
        validator = {
          check = val: if val == null then nullable else (mkCaseInsensitiveValidator allValues).check val;
          list = allValues;
        };
        resolve =
          value:
          if value == null then
            null
          else if hasAttr value aliases then
            aliases.${value}
          else
            value;
      };
