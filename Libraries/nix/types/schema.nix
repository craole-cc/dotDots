...
let
  ...
  extractDefaults =
    ...
    mapAttrs (
      _name: spec:
      spec.default or (
        if spec.type == "submodule" then
          extractDefaults spec.schema
        else if spec.type == "lazy" then
          null
        else
          null
      )
  ...
  validateField =
    ...
    if spec ? validate then
      spec.validate value
    else if spec.check value then
      value
    else
      throw "Field '${name}': validation failed for type ${spec.type}";
  ...
  validate =
    ...
    in
    if unknownKeys != [ ] then
      throw "Unknown fields: ${concatStringsSep ", " unknownKeys}"
    else
      mapAttrs (
        name: spec:
        if config ? ${name} then
          validateField name config.${name} spec
        else
          spec.default or (throw "Required field '${name}' is missing")
      ) schema;
  ...
  validateSoft =
    ...
    let
      results = mapAttrs (
        name: spec:
        if config ? ${name} then
          if spec.check config.${name} then
            {
              success = true;
              value = config.${name};
            }
  ...
  makeRequired =
    fields: schema: mapAttrs (name: spec: if elem name fields && spec.type == "nullOr" then spec.subtype else spec) schema;
    ...
    let
      ...
      validated = mapAttrs (
        ...
        if args ? ${argName} then
          ...
          throw "${name}: unexpected argument '${argName}'"
      ) namedArgs;
    in
    impl validated;
in
