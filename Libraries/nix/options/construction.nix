...
let
  ...
  */
  toOptionType =
    customTypeOrName:
    let
      customType = if isString customTypeOrName then customTypes.${customTypeOrName} else customTypeOrName;
      primitives = {
        "string" = str;
        "str" = str;
        "bool" = bool;
    ...
    in
    primitives.${customType.type} or (mkOptionType {
  ...
  */
  mkEnum =
    ...
    let
      e =
        if isAttrs input && input ? allValues then
          input
        else
          mkEnumData {
            values = input;
      ...
      elemType = if isNullable then nullOr (enum e.allValues) else enum e.allValues;
    in
    mkOption {
      default = if many then (if default == null then [ ] else default) else default;
      type = if many then listOf elemType else elemType;
    }
    // optionalDesc description;
