...
let
  ...
  mkConfigurations =
      ...
      */
      mkSystem =
        ...
        if class == "darwin" then
          evaluated // { system = evaluated.config.system.build.toplevel; }
        else
          evaluated;
    ...
    optionalAttrs (types.nixos != { }) {
      nixosConfigurations = mapAttrs (_: mkSystem) types.nixos;
    }
  ...
  */
  ...
  mkConfig =
    ...
    let
      ...
      condition = if predicate != null then predicate else cfg.enable or true;
  ...
  mkContext =
    {
      ...
      sub ? null,
      mod,
      kind ? if sub == "core" then dom else null,
      name ? mod,
    }:
    let
      self = rec {
