...
let
  ...
  exports = rec {
    ...
    external = { inherit (internal) mkUserPath mkSessionPaths; };
  };
  ...
  */
  resolve =
    ...
    in
    if hasPrefix "/" relative || hasPrefix "root:" relative then
      relative
    else if hasPrefix "dots:" relative || hasPrefix "$DOTS/" relative then
      dots + "/" + removePrefix "dots:" (removePrefix "$DOTS/" relative)
    else if hasPrefix "home:" relative || hasPrefix "$HOME/" relative then
            ...
            let
              ...
              rotation =
                if transformation == 1 then
                  90
                else if transformation == 2 then
                  180
                else if transformation == 3 then
                  270
                else
                  0;
