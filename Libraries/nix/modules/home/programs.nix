...
let
  ...
  __exports = rec {
    ...
    external = { inherit (internal) mkHomeApp mkHomeApps mkHomePrograms; };
  };

  normalizeName = value: if value == null then null else replaceStrings [ " " "_" ] [ "-" "-" ] (toLower value);
  ...
  mkApps =
    ...
    let
      ...
      appSpecs = {
        plasma.condition = { names, de, ... }: isIn names de;
        ...
        "zen-browser" =
          let
            variant = if hasInfix "twilight" firefox || firefox == "zen" then "twilight" else "beta";
          in
          {
            inherit variant;
            condition =
    ...
    let
      ...
      getCommand =
        ...
        let
          ...
          cmd =
            if category == "browser" && isIn zenNames n then
              attrByPath [
                "browser"
                (if hasInfix "twilight" n || n == "zen" then "zen-twilight" else "zen-beta")
              ] n mappings.command
            else if category == "editor" && hasInfix "code" n then
              attrByPath [ "editor" "vscode" ] "code" mappings.command
            else if category == "editor" && hasInfix "zed" n then
              attrByPath [ "editor" "zed" ] "zeditor" mappings.command
            else
              attrByPath [ category n ] n mappings.command;
        in
        if isIn n programs.tui then "${primaryTerminalCmd} -e ${cmd}" else cmd;
      ...
      getClass =
        ...
        in
        if hasInfix "fuzzel" n then
          "fuzzel"
        else if hasInfix "vicinae" n then
          "vicinae"
        else if hasInfix "yazi" n then
          "yazi"
        else
          attrByPath [ n ] n mappings.class;
