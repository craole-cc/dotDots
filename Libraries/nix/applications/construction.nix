    ...
    let
      ...
      mainApp = pkgs.writeShellApplication {
        ...
        inherit
          ...
          runtimeEnv
          ;
        meta = meta // optionalAttrs (description != null) { inherit description; };
        passthru = passthru // {
  ...
  mkScriptWrappers =
    ...
    mapAttrsToList (
      ...
      mkScriptWrapper (
        ...
        // (
          if isPath value || isString value then { script = value; } else value # already an attrset with { script, extraArgs?, ... }
        )
      )
    ) scripts;
in
