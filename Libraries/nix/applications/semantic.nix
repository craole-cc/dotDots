...
{_, primitives, predicates, builders, selectors}: let
  ...
  mkMaturityGroup   = {set}: mkEqQueries    {inherit set; field = "maturity";};
  mkProtocolGroup   = {set}: mkMemberQueries {inherit set; field = "protocol";};
  mkScopeGroup      = {set}: mkEqQueries    {inherit set; field = "scope";};
  ...
  mkMaturityQueries   = {set}: mkNamedQueries {prefix = "is";  set = mkMaturityGroup   {inherit set;};};
  mkProtocolQueries   = {set}: mkNamedQueries {prefix = "for"; set = mkProtocolGroup   {inherit set;};};
  mkScopeQueries      = {set}: mkNamedQueries {prefix = "as";  set = mkScopeGroup      {inherit set;};};
  mkCapabilityQueries = {set}: mkNamedQueries {prefix = "has"; set = mkCapabilityGroup {inherit set;};};

  # present only when field exists in set; produces { independent = …; integrated = …; }
  mkIndependenceQueries = {set}: let field = "independent"; in
    optionalAttrs (hasField {inherit field set;})
    (mkBoolQueries {inherit field set; trueKey = field; falseKey = "integrated";});
...
in {
  inherit
    mkMaturityGroup mkProtocolGroup mkScopeGroup mkCapabilityGroup
    mkMaturityQueries mkProtocolQueries mkScopeQueries mkCapabilityQueries
    mkIndependenceQueries mkConfigQueries
    ;

  # Applies every semantic query family; each is a no-op when its field is absent
  mkStandardQueries = {set, field ? null}:
