...
let
  ...
  /**
    ...
    ```
  */
  asPath = field: if isList field then field else splitString "." field;
  ...
  */
  toName =
    ...
    let
      ...
      resolvedPrefix = if prefix != null then prefix else optionalString (kind == "group") "by";
    in
  ...
  /**
    ...
    ```
  */
  normalizeOptional = value: if value == null || value == "" || value == "none" then null else value;
  ...
  */
  normalizeList =
    values: if isList values then filter (value: value != null && value != "" && value != "none") values else [ ];
