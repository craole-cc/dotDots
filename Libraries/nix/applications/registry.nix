...
let
  ...
  */
  lookup =
    ...
    in
    if elem category (app.categories or [ ]) then
      app
    else
      throw "'${name}' does not satisfy category '${category}'. Its categories: ${toString (app.categories or [ ])}";
