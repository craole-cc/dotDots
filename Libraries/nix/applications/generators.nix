...
let
  meta =
    let
      ...
      '';
      functions = { inherit userApplication userApplicationConfig program; };
      exports = {
        local = functions;
        alias = { };
      };
  ...
  */
  userApplication =
    ...
    let
      ...
      #~@ Runtime Identity
      command =
        if customCommand != null then
          customCommand
        else if packageFound then
          let
          ...
          null; # > Return null if package not found

      basename = if command != null then baseNameOf command else null;
      ...
      #~@ Role Classification
      default = if category != null then user.applications.${kind}.${category} or null else user.applications.${kind} or null;
      ...
      isX11Available =
        if requiresX11 then
          config.services.xserver.enable or false # TODO: This check could fail if we're querying the home-mamager config
        else
          true;
      ...
      #~@ Environment
      var = toUpper kind;
      varWithCategory = if category != null then "${var}_${toUpper category}" else var;
      ...
      output = rec {
        ...
        status = if isAllowed then "✓ ALLOWED {${role}}" else "✗ BLOCKED";
        ...
        variables = if sessionVariables == { } then "│   Variables: none" else builtins.toJSON sessionVariables;
      ...
      #~@ Debug Output
    in
    if debug then builtins.trace output.debug export else export;
