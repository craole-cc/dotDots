...
let
  ...
  */
  mkApp =
    ...
    let
      info =
        ...
        in
        if appInfo != null then
          if isFunction appInfo then appInfo { inherit pkgs inputs system; } else appInfo
        else
          {
            cmd = name;
            pkg = null;
      ...
      packages =
        ...
        let
          ...
          normalizedPrimary = if primary != null then detectBrowserVariant primary else null;
          normalizedSecondary = if secondary != null then detectBrowserVariant secondary else null;
          ...
          getPkg =
            i:
            if i.inputPath != null then
              if hasAttrByPath i.inputPath inputs then attrByPath i.inputPath null inputs else i.pkg
            else
              i.pkg;
      ...
      commands =
        ...
        let
          ...
          primaryInfo =
            if normalizedPrimary != null then
              info {
          ...
          secondaryInfo =
            if normalizedSecondary != null then
              info {
                inherit pkgs inputs system;
        ...
        in
        {
          primary = if primaryInfo != null then primaryInfo.cmd else null;
          secondary = if secondaryInfo != null then secondaryInfo.cmd else null;
        };
    in
    {
  ...
  detectBrowserVariant =
    ...
    if hasInfix "zen" lowerInput && (hasInfix "beta" lowerInput || hasInfix "nightly" lowerInput) then
      "zen-beta"
    else if hasInfix "zen" lowerInput then
      "zen-twilight"
    # Edge/Chromium variants
    else if hasInfix "edge" lowerInput then
      "edge"
    else if hasInfix "chrome" lowerInput && hasInfix "google" lowerInput then
      "chrome"
    else if hasInfix "chromium" lowerInput then
      "chromium"
    # Other browsers
    else if hasInfix "brave" lowerInput then
      "brave"
    else if hasInfix "vivaldi" lowerInput then
      "vivaldi"
    else if hasInfix "floorp" lowerInput then
      "floorp"
    else if hasInfix "firefox" lowerInput then
      "firefox"
