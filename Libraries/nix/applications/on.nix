...
let
  meta =
    let
      ...
      '';
      functions = { inherit byEnvironment mkFocus; };
      exports = {
  ...
  preferredMatch =
    env: candidates:
    let
      preferred = if isIn "wayland" (envProtocol env) then "app_id" else "class";
      match = filter (m: m.type == preferred) candidates;
    in
    if match != [ ] then head match else head candidates;
  ...
  mkWmctrl =
    ...
    let
      m = head (identify app);
    in
    "sh -c 'wmctrl -a \"${m.value}\" || ${app.exec}'";
  ...
  cosmic =
    ...
    in
    "sh -c 'cosmic-comp-msg focus-window --${m.type} \"${m.value}\" || ${app.exec}'";
  ...
  hyprland =
    ...
    in
    "sh -c 'if ${check}; then hyprctl dispatch focuswindow \"${m.type}:${m.value}\"; else ${app.exec}; fi'";
  ...
  niri =
    ...
    in
    "sh -c 'niri msg action focus-window --match ${m.type}=\"${m.value}\" || ${app.exec}'";

  sway =
    app:
    let
      m = preferredMatch "sway" (identify app);
      criterion = if m.type == "class" then "class" else "app_id";
    in
    "sh -c 'swaymsg [${criterion}=\"${m.value}\"] focus || ${app.exec}'";
  ...
  awesome =
    ...
    let
      m = head (identify app);
    in
    "sh -c 'awesome-client \"for c in client.get() do if c.${m.type} == \\\"${m.value}\\\" then c:jump_to() return end end\" || ${app.exec}'";
  ...
  i3 =
    ...
    let
      m = head (identify app);
    in
    "sh -c 'i3-msg [${m.type}=\"${m.value}\"] focus || ${app.exec}'";
  ...
  gnome =
    ...
    let
      m = head (identify app);
    in
    "sh -c 'gdbus call --session --dest org.gnome.Shell --object-path /org/gnome/Shell --method org.gnome.Shell.Eval \"global.get_window_actors().find(a => a.meta_window.get_${m.type}() === \\\"${m.value}\\\").meta_window.activate(0)\" || ${app.exec}'";
  ...
  plasma =
    ...
    let
      m = head (identify app);
    in
    "sh -c 'qdbus org.kde.KWin /KWin org.kde.KWin.activateWindow $(qdbus org.kde.KWin /KWin org.kde.KWin.getWindowInfo | grep -B5 \"${m.value}\" | grep windowId | awk \"{print \\$2}\") || ${app.exec}'";
