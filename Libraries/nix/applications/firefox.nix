...
let
  ...
  */
  extensionSettings = mapAttrs (_: entry: if isAttrs entry then entry else extensionEntry { id = entry; });
  ...
  */
  detectVariant =
    ...
    else if (hasInfix "zen" input && (hasInfix "beta" input || hasInfix "nightly" input || hasInfix "unstable" input)) then
      "zen-beta"
    else if (hasInfix "zen" input) || (hasInfix "twilight" input) then
      "zen-twilight"
    #~@ Check for LibreWolf
    else if
      (elem input [
    ...
    else if (elem input beta) then
      "firefox-beta"
    else if (elem input stable) then
      "firefox-esr"
    else if (elem input dev) then
      "firefox-devedition"
    else
      "firefox";
  ...
  resolveModule =
    ...
    let
      ...
      detectedVariant = detectVariant (
        if isNotEmpty variant then
          if (policies.dev or false) || (policies.devGui or false) then "${variant} dev" else variant
        else
          null
      );
  ...
  */
  zenVariant =
    ...
    if !isZen then
      null
    else if hasInfix "beta" detectedVariant then
      "beta"
    else
      "twilight";
in
