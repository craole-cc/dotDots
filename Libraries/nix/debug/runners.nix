      ...
      if isTest test then
        {
          ...
          error =
            if !test.passed then
              "Test `${name}` failed: expected ${builtins.toJSON test.desired}, got ${builtins.toJSON test.result}"
            else
              null;
        }
      else if isAttrs test then
        runTests test
      else
        test
    ) tests;
      ...
      mapAttrsToList (
        ...
        if isTest result then
          if !result.passed then
            ...
            [ ]
        else if isAttrs result then
          map (f: f // { path = "${name}.${f.path}"; }) (collectFailures result)
        else
          [ ]
      ) results
    );
