...
let
  ...
  renderType = value: if typeOf value == "lambda" then "function" else typeOf value;
  ...
  */
  trace =
    ...
    let
      prefix = if label == null || label == "" then "" else "${label} ";
      shownType = if displayType != null then displayType else renderType value;
      shownValue = if displayValue != null then displayValue else renderDebugValue value;
    in
  ...
  /**
    ...
    ```
  */
  traceRaw = { value, result }: lib.debug.trace value result;
