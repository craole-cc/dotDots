...
let
  ...
  */
  mkRef =
    ...
    if name != null then
      ...
      concatStringsSep "." (path ++ [ function ])
    else
      throw "mkRef: either `path` or `name` must be provided";
  ...
  */
  ...
  renderExample =
    ex:
    if isExample ex then
      "${ex.command}  # => ${ex.result}"
    else
      throw "debug.format.renderExample: value must be created with mkExample";
  ...
  */
  mkUsage =
    ...
    let
      ...
      body = "${ref}\nerror: ${message}\ninput: ${toJSON input}";
      ex = if example != null then renderExample example else null;
    in
    if signature == null || ex == null then
      body
    else
      "${body}\nusage:\n <{info}> repl> :doc ${ref}\n <{type}> ${signature}\n <{demo}> ${ex}";
