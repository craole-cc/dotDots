  ...
  */
  getFlakePath =
    ...
    let
      ...
      fileStr = "/flake.nix";
      result =
        if hasSuffix fileStr pathStr && pathExists pathStr then
          dirOf pathStr
        else if pathExists (pathStr + fileStr) then
          pathStr
        else
          null;
    in
    if (flake != null) && (flake ? outPath) then
      flake.outPath
    else
      traceIf (result == null) "❌ '${pathStr}' is not a valid flake path." result;
