...
let
  ...
  */
  concat =
    { root, stem }:
    let
      normalize = part: if isList part then concatStringsSep "/" part else part;
    in
    "${normalize root}/${normalize stem}";
  ...
  */
  mkPath =
    ...
    let
      ...
      unpacked = if isAttrs arg then arg else { path = arg; };
      raw = unpacked.path or arg;

      #> Normalise raw → absolute local string
      localStr =
        if isList raw then
          "${toString src}/${concatStringsSep "/" raw}"
        else if isPath raw then
          toString raw
        else if isString raw && hasPrefix "/" raw then
          raw
        else
          "${toString src}/${raw}";

      #> Resolve store path, null-safe
      storePath = if pathExists localStr then /. + localStr else null;
    in
    {
      store = storePath;
      local = localStr;
  ...
  construct =
    ...
    let
      normalized =
        if isAttrs arg then
          arg
        else if isString arg || isList arg then
          { stem = arg; }
        else
          throw "construct: expected attrset, string, or list — got: ${typeOf arg}";
      ...
      joinPath =
        ...
        if basePath == null then
          ...
          basePath + ("/" + (if isList stem then concatStringsSep "/" stem else stem));
    in
    {
