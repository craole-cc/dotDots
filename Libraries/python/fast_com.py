...
def fast_com(verbose=False, maxtime=15, forceipv4=False, forceipv6=False):
    ...
    try:
        ...
        if verbose:
            print(f"Error opening javascript URL: {e}")
        return 0
    ...
    if forceipv4:
        ...
        baseurl = (
            "http://" + ipv4 + "/"
        )  # HTTPS does not work with IPv4 addresses, thus use HTTP
    elif forceipv6:
        # force IPv6
    ...
    except (OSError, urllib2.URLError) as e:
        # not good
        if verbose:
            print(f"No connection possible: {e}")  # probably IPv6, or just no network
    ...
    for loop in range(nrloops):
        ...
        lasttotal = total
        highestspeedkBps = max(highestspeedkBps, speedkBps)
