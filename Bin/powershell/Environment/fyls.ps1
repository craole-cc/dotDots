...
function Invoke-Fyls {
    ...
    if ($ShowCommand) {
        ...
        Invoke-Fyls-Command -Command $command
    }
}
function Invoke-Fyll {
    ...
    Invoke-Fyls -DirFirst -Icons -Git -All -Long @RemainingArguments
}
...
function Initialize-FylsConfig {
    ...
    $config = @{
        ...
        # Depth         = $Parameters.Depth -or (if ($Parameters.Tree) { 5 } else { 0 })
