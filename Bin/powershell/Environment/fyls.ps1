...
function Global:Invoke-Fyls {
  ...
  param(
    ...
    [Alias('page', 'bat', 'more', 'less','paginate')]
    [Parameter()]
    [switch]$Pagination,
  ...
  foreach ($path in $Path) {
    #TODO: Finish this, Why does Resolve-PathSafely not work
    # $resolvedPath = Resolve-PathSafely -NoClobber -Path $path
    Write-Host "Path is $path"
    $resolvedPath = Resolve-Path -Path $path
    Write-Host "Resolved path is $resolvedPath"
    if ($resolvedPath) {
  ...
  if ($ShowCommand) {
    ...
    try {
      Write-Pretty -Tag 'Info' -NoNewLine -ContextScope Name `
...
function Global:Initialize-FylsConfig {
  ...
  $config = @{
    ...
    Depth         = if ($Parameters.Depth) { $Parameters.Depth } else { if ($Parameters.Tree) { 3 } else { 0 } }
...
function Global:Resolve-SortPreferences {
  <#
    .SYNOPSIS
...
function Global:Resolve-DisplayPreferences {
  <#
    .SYNOPSIS
...
function Global:Get-BestAvailableTool {
  <#
    .SYNOPSIS
...
function Global:Test-ToolAvailability {
  <#
    .SYNOPSIS
...
function Global:Build-EzaCommand {
  <#
    .SYNOPSIS
...
function Global:Build-LsdCommand {
  <#
    .SYNOPSIS
...
function Global:Build-LsCommand {
  <#
    .SYNOPSIS
...
function Global:Build-PowerShellCommand {
  <#
    .SYNOPSIS
...
function Global:Add-Pagination {
  <#
    .SYNOPSIS
