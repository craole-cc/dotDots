...
function Set-MiseEnv
{
    ...
    if ($miseCmd)
    {
        ...
        $cmdPath = $miseCmd.Source.Replace('\', '\\')
    ...
    {
        $escapedRcPath = Format-Path $rcStr
        Set-Content `
            -Path $rcStr `
    ...
    $escapedRcPath = Format-Path $rcPath
    Set-Env -Name 'MISE_RC' -Target $escapedRcPath
...
# Set environment variable CMD_MISE
Set-Env -Type env -Name 'TEST_CMD_MISE' -Target 'C:\\Path\\To\\mise.exe'

# Set alias m to Invoke-Mise
Set-Env -Type alias -Name 'test_m' -Target 'Invoke-Mise'
