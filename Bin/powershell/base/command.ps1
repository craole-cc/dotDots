...
.DESCRIPTION
    Executes a command, measures duration, and outputs formatted results using Write-Output.
.PARAMETER Command
...
#>
    ...
    # Write-Output -Context $Context -Scope $Scope POP -Tag 'Trace'
    ...
    #@ Output result using Write-Output
    Write-Output `
        -Verbosity $Tag `
        -Delimiter $Delimiter `
        -Duration $Runtime `
...
#>
    ...
    Write-Host "`nTest 2: Invoke-Process with Write-Output"
    Invoke-Process 'Write-Output' -Arguments 'Hello, world!' -Verbosity 'Debug'
