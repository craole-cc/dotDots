...
in {
  options._ = {
    ...
    output = lib.mkOption {
      ...
      default = {};
      # `anything` recursively to the module system's built-in merge: attrsets
      # merge recursively; list handling follows the inferred value type, so multiple modules can stage
