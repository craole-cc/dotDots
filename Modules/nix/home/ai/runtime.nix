{
  ...
  configureHermes = pkgs.writeShellScript "configure-hermes-hindsight" ''
    ...
    # Hermes refuses to load its own dotenv file unless it is private. The
    # file is generated by Hermes tooling outside the Nix store, so preserve
    # it but repair its mode on every declarative activation.
