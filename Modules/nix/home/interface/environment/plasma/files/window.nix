{
  kwinrc = {
    Desktops = {
      Id_1 = "3f37f39a-b4ec-4ceb-8c77-8ac4460f53f9";
      Number = 1;
      Rows = 1;
    };

    Effect-blur = {
      BlurStrength = 1;
      NoiseStrength = 2;
    };

    Effect-kwin4_effect_geometry_change = {
      Duration = 450;
    };

    ElectricBorders = {
      BottomLeft = "ApplicationLauncher";
      BottomRight = "ShowDesktop";
    };

    NightColor = {
      Active = true;
      NightTemperature = 4100;
    };

    Plugins = {
      blurEnabled = true;
      cubeEnabled = true;
      krohnkiteEnabled = false;
      kwin4_effect_geometry_changeEnabled = true;
      slidebackEnabled = true;
      synchronizeskipswitcherEnabled = true;
      touchpointsEnabled = true;
      translucencyEnabled = true;
      wobblywindowsEnabled = true;
    };

    ScreenEdges = {
      RemainActiveOnFullscreen = true;
    };

    Script-krohnkite = {
      binaryTreeLayoutOrder = 6;
      cascadeLayoutOrder = 7;
      columnsLayoutOrder = 9;
      floatingLayoutOrder = 10;
      monocleMinimizeRest = true;
      quarterLayoutOrder = 4;
      quarterLayoutReset = true;
      screenGapBetween = 4;
      spiralLayoutOrder = 3;
      spreadLayoutOrder = 11;
      stackedLayoutOrder = 8;
      stairLayoutOrder = 5;
      threeColumnLayoutOrder = 0;
    };

    TabBox = {
      BorderActivate = 1;
      OrderMinimizedMode = 1;
    };

    TabBoxAlternative = {
      HighlightWindows = false;
      MultiScreenMode = 1;
    };

    Tiling.padding = 4;
    "Tiling/14d2eb36-ff2f-42fc-8b2c-abf052ae4ffd/197399ba-7544-4c10-90db-dc25b0dd0244".tiles =
      "{\"layoutDirection\":\"horizontal\",\"tiles\":[{\"width\":0.25},{\"width\":0.5},{\"width\":0.25}]}";
    "Tiling/14d2eb36-ff2f-42fc-8b2c-abf052ae4ffd/1f334bef-499d-4daf-9dc4-3a7fb49ea389".tiles =
      "{\"layoutDirection\":\"horizontal\",\"tiles\":[{\"width\":0.25},{\"width\":0.5},{\"width\":0.25}]}";
    "Tiling/14d2eb36-ff2f-42fc-8b2c-abf052ae4ffd/b49616c0-ed3b-4b52-96bb-371b57ef64ab".tiles =
      "{\"layoutDirection\":\"horizontal\",\"tiles\":[{\"width\":0.25},{\"width\":0.5},{\"width\":0.25}]}";
    "Tiling/3040216e-31b3-41b6-9b91-767980dc298f/197399ba-7544-4c10-90db-dc25b0dd0244".tiles =
      "{\"layoutDirection\":\"horizontal\",\"tiles\":[{\"width\":0.25},{\"width\":0.5},{\"width\":0.25}]}";
    "Tiling/3040216e-31b3-41b6-9b91-767980dc298f/1f334bef-499d-4daf-9dc4-3a7fb49ea389".tiles =
      "{\"layoutDirection\":\"horizontal\",\"tiles\":[{\"width\":0.25},{\"width\":0.5},{\"width\":0.25}]}";
    "Tiling/3040216e-31b3-41b6-9b91-767980dc298f/b49616c0-ed3b-4b52-96bb-371b57ef64ab"."[Tiling][3f37f39a-b4ec-4ceb-8c77-8ac4460f53f9][]" =
      "";
    "Tiling/3040216e-31b3-41b6-9b91-767980dc298f/b49616c0-ed3b-4b52-96bb-371b57ef64ab".tiles =
      "{\"layoutDirection\":\"horizontal\",\"tiles\":[{\"width\":0.25},{\"width\":0.5},{\"width\":0.25}]}";
    "Tiling/3f37f39a-b4ec-4ceb-8c77-8ac4460f53f9/197399ba-7544-4c10-90db-dc25b0dd0244".tiles =
      "{\"layoutDirection\":\"horizontal\",\"tiles\":[{\"width\":0.25},{\"layoutDirection\":\"vertical\",\"tiles\":[{\"height\":0.5},{\"height\":0.5}],\"width\":0.5},{\"width\":0.25}]}";
    "Tiling/3f37f39a-b4ec-4ceb-8c77-8ac4460f53f9/1f334bef-499d-4daf-9dc4-3a7fb49ea389".tiles =
      "{\"layoutDirection\":\"horizontal\",\"tiles\":[{\"width\":0.25},{\"width\":0.5},{\"width\":0.25}]}";
    "Tiling/3f37f39a-b4ec-4ceb-8c77-8ac4460f53f9/4a75fc45-66ef-457e-8112-c3c5c4db2d71".tiles =
      "{\"layoutDirection\":\"horizontal\",\"tiles\":[{\"width\":0.25},{\"width\":0.5},{\"width\":0.25}]}";
    "Tiling/3f37f39a-b4ec-4ceb-8c77-8ac4460f53f9/6b27df1b-6f11-48f3-8bb6-97a7a603f165".tiles =
      "{\"layoutDirection\":\"horizontal\",\"tiles\":[{\"width\":0.25},{\"width\":0.5},{\"width\":0.25}]}";
    "Tiling/3f37f39a-b4ec-4ceb-8c77-8ac4460f53f9/b49616c0-ed3b-4b52-96bb-371b57ef64ab".tiles =
      "{\"layoutDirection\":\"horizontal\",\"tiles\":[{\"width\":0.25},{\"width\":0.5},{\"width\":0.25}]}";
    "Tiling/c9facf33-0553-4287-8aa1-f0e60ce2e05e/197399ba-7544-4c10-90db-dc25b0dd0244".tiles =
      "{\"layoutDirection\":\"horizontal\",\"tiles\":[{\"width\":0.25},{\"width\":0.5},{\"width\":0.25}]}";
    "Tiling/c9facf33-0553-4287-8aa1-f0e60ce2e05e/1f334bef-499d-4daf-9dc4-3a7fb49ea389".tiles =
      "{\"layoutDirection\":\"horizontal\",\"tiles\":[{\"width\":0.25},{\"width\":0.5},{\"width\":0.25}]}";
    "Tiling/c9facf33-0553-4287-8aa1-f0e60ce2e05e/b49616c0-ed3b-4b52-96bb-371b57ef64ab".tiles =
      "{\"layoutDirection\":\"horizontal\",\"tiles\":[{\"width\":0.25},{\"width\":0.5},{\"width\":0.25}]}";
    "Tiling/dc73a97e-df42-4806-a3b2-d317abe7515a/197399ba-7544-4c10-90db-dc25b0dd0244".tiles =
      "{\"layoutDirection\":\"horizontal\",\"tiles\":[{\"width\":0.25},{\"width\":0.5},{\"width\":0.25}]}";
    "Tiling/dc73a97e-df42-4806-a3b2-d317abe7515a/1f334bef-499d-4daf-9dc4-3a7fb49ea389".tiles =
      "{\"layoutDirection\":\"horizontal\",\"tiles\":[{\"width\":0.25},{\"width\":0.5},{\"width\":0.25}]}";
    "Tiling/dc73a97e-df42-4806-a3b2-d317abe7515a/b49616c0-ed3b-4b52-96bb-371b57ef64ab".tiles =
      "{\"layoutDirection\":\"horizontal\",\"tiles\":[{\"width\":0.25},{\"width\":0.5},{\"width\":0.25}]}";
    Xwayland.Scale = 1;
    "org.kde.kdecoration2".theme = "__aurorae__svg__CatppuccinFrappe-Modern";
  };

  kwinrulesrc = {
    "10f83db4-2d75-4d8b-b2c5-4fcf83e82503" = {
      Description = "Application settings for systemsettings";
      Enabled = false;
      opacityactive = 97;
      opacityactiverule = 2;
      wmclass = "systemsettings";
      wmclassmatch = 1;
    };

    "2e3d1156-1fba-4374-b3fe-83681f02fdab" = {
      Description = "Application settings for warp";
      Enabled = false;
      opacityactive = 97;
      opacityactiverule = 2;
      wmclass = "warp";
      wmclassmatch = 2;
    };

    "72a3a5bf-cd04-4493-ab1e-1160d78c771e" = {
      Description = "Application settings for org.kde.dolphin";
      Enabled = false;
      opacityactive = 97;
      opacityactiverule = 2;
      wmclass = "dolphin org.kde.dolphin";
      wmclasscomplete = true;
      wmclassmatch = 1;
    };

    General = {
      count = 8;
      rules = "d8da0706-fa50-4541-9491-246c306efbff,72a3a5bf-cd04-4493-ab1e-1160d78c771e,c801bfc4-1ff0-4374-ad6e-5fc76709f6fc,ed1c1ff5-5be9-45b5-b474-d8d3058cd8f7,bcf2ed01-e8d2-406a-a3ed-68f5f0886dd9,e9c1ef02-f796-4872-8589-e5428cbfaef1,2e3d1156-1fba-4374-b3fe-83681f02fdab,10f83db4-2d75-4d8b-b2c5-4fcf83e82503";
    };

    bcf2ed01-e8d2-406a-a3ed-68f5f0886dd9 = {
      Description = "Application settings for code";
      Enabled = false;
      opacityactive = 98;
      opacityactiverule = 2;
      wmclass = "code";
      wmclassmatch = 1;
    };

    c801bfc4-1ff0-4374-ad6e-5fc76709f6fc = {
      Description = "Application settings for org.kde.kinfocenter";
      Enabled = false;
      opacityactive = 97;
      opacityactiverule = 2;
      wmclass = "systemsettings org.kde.kinfocenter";
      wmclasscomplete = true;
      wmclassmatch = 1;
    };

    d8da0706-fa50-4541-9491-246c306efbff = {
      Description = "Application settings for koi";
      Enabled = false;
      opacityactive = 97;
      opacityactiverule = 2;
      wmclass = "koi";
      wmclassmatch = 1;
    };

    e9c1ef02-f796-4872-8589-e5428cbfaef1 = {
      Description = "Application settings for yakuake";
      Enabled = false;
      opacityactive = 97;
      opacityactiverule = 2;
      wmclass = "yakuake";
      wmclassmatch = 2;
    };

    ed1c1ff5-5be9-45b5-b474-d8d3058cd8f7 = {
      Description = "Application settings for zen-twilight";
      Enabled = false;
      opacityactive = 97;
      opacityactiverule = 2;
      wmclass = "zen zen-twilight";
      wmclasscomplete = true;
      wmclassmatch = 1;
    };
  };
}
