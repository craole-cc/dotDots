...
- Prefer reusable logic in `Libraries/nix` or `Modules/nix`; keep host- and
  user-specific choices in `API/nix`.
- Do not assume `default.nix` contains substantive logic. Many `default.nix`
...
- If the change is about one user’s apps, interface, or paths, start in
  `API/nix/users/<user>/`.
- If the change affects all hosts or all HM users, start in `Modules/nix`.
- If the change introduces a shared helper or abstraction, start in
  `Libraries/nix`.
...
- Adding one-off module logic when a library helper would make the pattern
  reusable.
