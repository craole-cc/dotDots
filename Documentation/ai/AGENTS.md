...
- This repo is a Lix-centered Nix flake for cross-system dotfiles, host
  definitions, user profiles, and reusable config modules.
- `flake.nix` is the flake output entrypoint.
- Top-level `default.nix` assembles `lix`, `tree`, `schema`, `hosts`, and
  `users`.
...
- Trace runtime flow as `flake.nix` -> `default.nix` ->
  `lix.modules.construction` -> `schema` / `tree` -> `API/nix` + `Modules/nix`.
