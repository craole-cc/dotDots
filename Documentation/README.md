...
```json
{
  "loadOrder": ["core.ps1", "utils.ps1", "helpers.ps1"]
}
```
...
```json
{
  "loadOrder": ["important.ps1", "secondary.ps1"],
  "excludes": ["debug*", "test*"]
}
```
...
When local configuration specifies a load order:
...
- File extension is `.toml` (parsing not yet implemented)

**Plain Text:**

- Everything else treated as plain text load order
