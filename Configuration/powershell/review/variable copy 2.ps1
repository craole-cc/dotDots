function Global:Get-Env {
    ...
    if ([string]::IsNullOrEmpty($Name) -or $ListAll) {
        ...
        if ($Scope -eq 'All' -or $ListAll) {
            # Get from all scopes with Process taking precedence
            $allEnvVars = @{}

            # Process scope takes highest precedence, then User, then Machine
            foreach ($scopeToCheck in @('Machine', 'User', 'Process')) {
                try {
                    Write-Pretty -Tag "Debug" -ContextScope $script:ctxScope -OneLine -Message "Attempting to access $scopeToCheck environment variables..."

                    # Try the original method first
                    $vars = [System.Environment]::GetEnvironmentVariables($scopeToCheck)
                    ...
                    Write-Pretty -Tag "Debug" -ContextScope $script:ctxScope -OneLine -Message "Successfully accessed $($vars.Count) variables from $scopeToCheck scope"
                }
                catch {
                    Write-Pretty -Tag "Debug" -ContextScope $script:ctxScope -OneLine -Message "Primary method failed for $scopeToCheck scope: $($_.Exception.Message)"
                    ...
                    try {
                        ...
                        Write-Pretty -Tag "Warning" -ContextScope $script:ctxScope -OneLine -Message "Could not access $scopeToCheck environment variables using any method: $($_.Exception.Message)"
                        continue
                    }
                }
            }
...
function Test-GetEnv {
    ...
    Write-Pretty -Tag "Info" -ContextScope $script:ctxScope -OneLine -Message "  User: $(Get-Env 'USERNAME')"
    Write-Pretty -Tag "Info" -ContextScope $script:ctxScope -OneLine -Message "  Domain: $(Get-Env 'USERDOMAIN')"
