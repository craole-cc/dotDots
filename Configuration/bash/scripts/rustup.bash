_rustup() {
  ...
  case "${cmd}" in
  rustup)
    opts="-v -q -h -V --verbose --quiet --help --version [+toolchain] install uninstall dump-testament toolchain default show update check target component override run which doc self set completions help"
  ...
  rustup__dump__testament)
    ...
    *)
    ...
    ;;
  rustup__help)
    opts="install uninstall dump-testament toolchain default show update check target component override run which doc self set completions help"
    if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]]; then
      COMPREPLY=($(compgen -W "${opts}" -- "${cur}"))
      return 0
    fi
