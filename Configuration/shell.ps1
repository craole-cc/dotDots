...
$script:ShellConfig = @{
  Verbose = $false
  Paths   = @{
    Base   = [IO.Path]::GetFullPath($env:DOTS)
    Bin    = [IO.Path]::Combine($env:DOTS, 'Bin')
    Config = @{
      dots = [IO.Path]::Combine($env:DOTS, 'Configuration')
      user = [IO.Path]::Combine($env:APPDATA)
    }
    Cache  = [IO.Path]::Combine($env:DOTS, 'Cache')
    data   = [IO.Path]::Combine($env:DOTS, 'Data')
  }
  Shells  = @{
    PowerShell = @{
      Name       = 'pwsh'
...
try {
  ...
  Write-Host "Detected shell: $currentShell"
